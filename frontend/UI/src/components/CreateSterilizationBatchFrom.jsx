...
} from "../services/sterilizationBatch.service";
...
export default function CreateSterilizationBatchFrom({
  ...
  useEffect(() => {
    if (!batchData.stBatch) return;
    const { stBatch } = batchData;

    setValue("id", stBatch.id);
    setValue("startdate", stBatch.startdate);
    setValue("enddate", stBatch.enddate);
    setValue("status", stBatch.status);
  }, [setValue, batchData]);
  ...
  const onSubmit = async (payload) => {
    ...
    const {
      ...
      ? await createSterilizationBatch(createPayload)
      : await updateSterilizationBatch(updatePayload);
      ...
      <div className="p-4 md:p-5">
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex gap-4">
            ...
            >
              {!batchData.stBatch ? "Registrar" : "Actulizar"}
            </button>
            <button
              type="button"
